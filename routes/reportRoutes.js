...
const normalizeModality = (value) => {
  ...
  return ALLOWED_MODALITIES.includes(modality)
    ? modality
    : null;
};
...
const normalizeEmotion = (value) => {
  ...
  return ALLOWED_EMOTIONS.includes(emotion)
    ? emotion
    : "unknown";
};
    ...
    try {
      ...
      if (!childId || !startDate || !endDate) {
        return res.status(400).json({
          message:
            "childId, startDate, and endDate are required",
        });
      }
      ...
      if (!child.parentId) {
        return res.status(422).json({
          message:
            "Child parent information is missing",
        });
      }
      ...
      if (!hasChildAccess) {
        return res.status(403).json({
          message:
            "You are not authorized to access this child report",
        });
      }
      ...
      const doctor =
        reportCase?.doctorId || null;

      const combinedDoctorName = [
        doctor?.firstName,
        doctor?.lastName,
      ]
        .filter(Boolean)
      ...
      for (const analysis of analyses) {
        ...
        if (!modality) {
          return res.status(422).json({
            message:
              "Stored analysis contains an unsupported modality",
          });
        }
        ...
        uniqueContexts.forEach((context) => {
          patternCounts[context] =
            (patternCounts[context] || 0) + 1;
        });
      ...
      const sortedEmotions = Object.entries(
        emotionCounts,
      ).sort(
        (
          firstEmotion,
      ...
      if (
        ...
        sortedEmotions[0][1] > 0
      ) {
        const highestEmotionCount =
          sortedEmotions[0][1];

        const mostFrequentEmotions =
          sortedEmotions.filter(
            ([, count]) =>
              count === highestEmotionCount,
          );
      ...
      const reportRequestData = {
        ...
        child: {
          childName:
            reportIdentity.childName,
          childId:
            reportIdentity.childId.toString(),
          parentId:
            reportIdentity.parentId.toString(),
          doctorName:
            reportIdentity.doctorName,
          doctorId: reportIdentity.doctorId
            ? reportIdentity.doctorId.toString()
            : null,
          caseId: reportIdentity.caseId
            ? reportIdentity.caseId.toString()
            : null,
        },
        period: {
      ...
      try {
        ...
        if (
          ...
          return res.status(504).json({
            message:
              "AI report service request timed out",
          });
        }
        ...
        if (!aiError.response) {
          return res.status(503).json({
            message:
              "AI report service is currently unavailable",
          });
        }

        return res.status(502).json({
          message:
            "AI report service failed to generate the report",
        });
      }

      const hasValidGeneratedReport =
        generatedReport &&
        typeof generatedReport.summary ===
          "string" &&
        generatedReport.summary.trim()
          .length > 0 &&
        typeof generatedReport.report ===
          "string" &&
        generatedReport.report.trim()
          .length > 0 &&
        Array.isArray(
          generatedReport.recommendations,
        ) &&
        typeof generatedReport.overallStatus ===
          "string";

      if (!hasValidGeneratedReport) {
        return res.status(502).json({
          message:
            "AI report service returned an invalid response",
        });
      }

      const overallStatus =
        generatedReport.overallStatus
          .trim()
          .toLowerCase();

      if (
        !ALLOWED_OVERALL_STATUSES.includes(
          overallStatus,
        )
      ) {
        return res.status(502).json({
          message:
            "AI report service returned an invalid status",
        });
      }
      ...
      const savedReport =
        await Report.create({
          childId:
            reportIdentity.childId,
          childName:
            reportIdentity.childName,
          parentId:
            reportIdentity.parentId,
          caseId:
            reportIdentity.caseId,
          doctorId:
            reportIdentity.doctorId,
          doctorName:
            reportIdentity.doctorName,
